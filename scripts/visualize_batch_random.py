...
OUTPUT_RRD = "rerun_logs/nuscenes_batch_random_50.rrd"
NUM_SCENES = 50
...
def main():
    ...
    # Select 50 Random Scenes
    all_scenes = list(scenes_map.keys())
    if len(all_scenes) > NUM_SCENES:
        selected_scenes = random.sample(all_scenes, NUM_SCENES)
    else:
        selected_scenes = all_scenes
    
    print(f"Selected {len(selected_scenes)} random scenes from {len(all_scenes)} total.")

    rr.init("NuScenes Random Batch", spawn=False)
    
    global_step = 0
    maps = {}

    for st in tqdm(selected_scenes, desc="Processing Scenes"):
        rows = scenes_map[st]
        
        try:
            scene_rec = nusc.get("scene", st)
        except:
            print(f"Warning: Scene {st} not found in NuScenes DB. Skipping.")
            continue
        ...
        for i, row in enumerate(rows):
            ...
            if waypoints:
                ...
                # --- NEW LABEL LOGIC (Index 5) ---
                if len(wps_ego) > 5:
                    midpoint_ego = wps_ego[5]
                elif len(wps_ego) > 0:
            ...
            # 9. Maneuver/Action Label (FLOATING)
