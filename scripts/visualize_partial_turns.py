...
# Point to the ongoing/partial 12s CSV or the completed one
CSV_PATH = "nuscenes_goal_tokens_12s_merged.csv" 
OUTPUT_DIR = "bev_frames_partial_turns"
SCENES_PER_ACTION = 5 # "Few samples"
TARGET_ACTIONS = ["LEFT", "RIGHT"] # Keywords to filter
# ==========================================

def render_bev_frame(nusc, row, save_path):
    fig, ax = plt.subplots(figsize=(10, 10))
    
    # 1. Start from basic data
    sample_token = row['sample_token']
    try:
        sample = nusc.get('sample', sample_token)
    except:
        # If partial CSV has tokens not in this split, skip
        plt.close(fig)
        return
    ...
    if waypoints_str and waypoints_str != "[]":
        try:
            ...
            # Action Label at Start (Index 5 or 0)
            lbl_pt = None
            if len(wps_ego) > 5: lbl_pt = wps_ego[5]
            elif len(wps_ego) > 0: lbl_pt = wps_ego[0]
    ...
    # 6. HUD
    gt = row.get('goal_token', '-')
    mt = row.get('maneuver_type', '-')
    at = row.get('action_token', '-')
    
    info_text = f"Goal: {goal_name} ({gt})\nManeuver: {mt}\nAction: {at}\nSample: {sample_token}"
...
def main():
    ...
    with open(CSV_PATH, 'r') as f:
        ...
        for row in reader:
            ...
            # Check keywords
            if any(k in at for k in TARGET_ACTIONS):
                # We want unique scenes per action bucket
                if at not in action_scenes:
                    action_scenes[at] = {} # st -> [rows]
                
                if st not in action_scenes[at]:
                    action_scenes[at][st] = []
                action_scenes[at][st].append(row)
    
    # 2. Select Scenes
    for action in action_scenes:
        scenes = list(action_scenes[action].keys())
        # Pick Few
        selected_scenes = scenes[:SCENES_PER_ACTION]
        
        print(f"Generating for {action} ({len(selected_scenes)} scenes)...")
        
        action_dir = os.path.join(OUTPUT_DIR, action)
        if not os.path.exists(action_dir):
            os.makedirs(action_dir)
            
        for st in tqdm(selected_scenes, desc=action):
            scene_dir = os.path.join(action_dir, st)
            if not os.path.exists(scene_dir):
                os.makedirs(scene_dir)
            
            rows = action_scenes[action][st]
